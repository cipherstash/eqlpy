...
class EqlTest(unittest.TestCase):
    ...
    def test_from_parsed_json_uses_p_value(self):
        ...
        self.assertEqual(
            EqlInt.from_parsed_json(self.template_dict),
            1
        )
    ...
    def test_eql_bool_from_parsed_json_true(self):
        ...
        self.assertEqual(
            EqlBool.from_parsed_json(self.template_dict),
            True
        )
    ...
    def test_eql_date_from_parsed_json(self):
        ...
        self.assertEqual(
            EqlDate.from_parsed_json(self.template_dict),
            date(2024, 11, 1)
        )
    ...
    def test_eql_float_from_parsed_json(self):
        ...
        self.assertEqual(
            EqlFloat.from_parsed_json(self.template_dict),
            1.1
        )
    ...
    def test_eql_text_from_parsed_json(self):
        ...
        self.assertEqual(
            EqlText.from_parsed_json(self.template_dict),
            "text"
        )
    ...
    def test_eql_jsonb_prints_json_string(self):
        ...
        self.assertEqual(
            eql_jsonb._value_in_db_format("ste_vec"),
            '{"a": 1}'
        )
    ...
    def test_eql_jsonb_prints_value_for_ejson_path(self):
        ...
        self.assertEqual(
            eql_jsonb._value_in_db_format("ejson_path"),
            '$.a.b'
        )

    def test_eql_jsonb_returns_value(self):
        self.assertEqual(
            EqlJsonb._value_from_db_format('{"a": 1}'),
            {"a": 1}
        )
    ...
    def test_eql_row_makes_row(self):
        ...
        eql_row = EqlRow(
            column_function_mapping,
            {"encrypted_int": json.loads(EqlInt(1, "table", "column").to_db_format()),
             "encrypted_boolean": json.loads(EqlBool(True, "table", "column").to_db_format()),
             "encrypted_date": json.loads(EqlDate(date(2024, 11, 1), "table", "column").to_db_format()),
             "encrypted_float": json.loads(EqlFloat(1.1, "table", "column").to_db_format()),
             "encrypted_utf8_str": json.loads(EqlText("text", "table", "column").to_db_format()),
             "encrypted_jsonb": json.loads(EqlJsonb('{"a": 1}', "table", "column").to_db_format())
            })
